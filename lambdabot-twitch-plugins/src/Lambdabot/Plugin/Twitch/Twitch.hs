  ...
  { moduleCmds     =
    return
      [ (command "twitch-connect")
        ...
        , help       =
          say
            "twitch-connect tag host portnum nickname userinfo.  connect to a twitch server"
      ...
      , (command "twitch-persist-connect")
        ...
        , help       =
          say
            "twitch-persist-connect tag host portnum nickname userinfo.  connect to a twitch server and reconnect on network failures"
        , process    =
      ...
      , (command "twitch-auth")
        { privileged = True
        , help       =
          say "twitch-auth oauth.  set oauth for next twitch-connect command"
        , process    = \rest -> case splitOn " " rest of
...
decodeMessage svr lbn line = IrcMessage { ircMsgServer  = svr
                                        , ircMsgLBName  = lbn
                                        , ircMsgPrefix  = prefix
                                        , ircMsgCommand = cmd
                                        , ircMsgParams  = params
 ...
 where
  (prefix, rest1) = decodePrefix (,) line
...
decodeParams' param params [] | null param = reverse params
...
doRetry hostn portnum tag delay nickn psw ui = do
  continue <- lift $ gets $ \st ->
    (M.member tag $ ircPersists st) && not (M.member tag $ ircServerMap st)
  if continue
    then E.catch
      (goOnline hostn portnum tag nickn psw ui)
      (\e@SomeException{} -> do
        errorM (show e)
      ...
      forM_ (M.keys chans) $ \chan ->
        when (nTag (getCN chan) == tag) $ lift $ modify $ \state' ->
...
goOnline hostn portnum tag nickn psw ui = do
  ...
  io . void . fork . forever $ do
    ...
    SSem.signal sem1
  fin <- io $ SSem.new 0
  E.catch (registerServer tag (io . sendMsg sock sendmv fin))
          (\err@SomeException{} -> io (hClose sock) >> E.throwIO err)
  lb $ twitchSignOn hostn (Nick tag nickn) psw ui
  ready <- io $ SSem.new 0
  lb $ void $ forkFinally
    (E.catch (readerLoop tag nickn sock ready)
...
sendMsg sock mv fin msg = E.catch
  (do
    takeMVar mv

module Lambdabot.Plugin.Core.System (
  systemPlugin,
) where
...
import Lambdabot.Module (
  ...
  ModuleInfo (moduleName, theModule),
  ModuleT,
  newModule,
 )
...
toggleNick :: (Ord a, MonadLB m) => ((a -> S.Set a -> S.Set a) -> Nick -> IRCRWState -> IRCRWState) -> String -> Cmd m ()
...
listModule s = inModuleNamed s fromCommand printProvides
 where
  fromCommand = withCommand s (return $ "No module \"" ++ s ++ "\" loaded") (const printProvides)
...
printProvides = do
  ...
  return . concat $ if null cmds' then [name', " has no visible commands"] else [name', " provides: ", showClean (concatMap cmdNames cmds')]

...
evalExpr (Cmd c args) = do
  ...
  let arg = concat $ concatMap (intersperse " ") args'
  cmd <- lookupP c
  lift (lift (cmd arg))
...
parseCommand, parseInlineCommand
...
parseString delim ('\\' : x : xs) =
  first (\ys -> '\\' : x : ys) (parseString delim xs)
parseString delim (x : xs) | delim == x = ([x], xs)
...
isCommand xs = msum . map dropPrefix
 where
  dropPrefix p | p `isPrefixOf` xs = Just $ drop (length p) xs
               | otherwise         = Nothing

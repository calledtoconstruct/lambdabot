...
instance Message IrcMessage where
  ...
  channels msg        = 
    ...
    in map (Nick (server msg)) $ map (\(x:xs) -> if x == ':' then xs else x:xs) (splitOn "," cstr)
...
timeReply msg = msg {
  ...
  ircMsgParams  = [ head (ircMsgParams msg), ":@localtime-reply " ++ (nName $ nick msg) ++ ":" ++ (init $ drop 7 (last (ircMsgParams msg))) ]
}

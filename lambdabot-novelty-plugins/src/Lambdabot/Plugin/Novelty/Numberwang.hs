...
module Lambdabot.Plugin.Novelty.Numberwang (
  numberwangPlugin,
) where
...
doNumberwang :: (Num a, Ord a, MonadLBState m, LBState m ~ NumberwangState) => Bool -> a -> Cmd m ()
doNumberwang cmd n
...
withState :: (MonadLBState m, LBState m ~ NumberwangState) => Bool -> (Int -> (Int -> m ()) -> RVar Int -> m a) -> m a
withState True f =
  withMS $ \st setST -> f (nextCmd st) (\n -> setST st{nextCmd = n}) cmdDist
withState False f = withMS $
  \st setST -> f (nextCon st) (\n -> setST st{nextCon = n}) conDist

checkNumberwang :: (MonadLBState m, LBState m ~ NumberwangState) => Bool -> Int -> m Bool
checkNumberwang cmd l = withState cmd $ \n setN nDist ->
  if n <= l
    then do

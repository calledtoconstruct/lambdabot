...
-- | Support for quotes
module Lambdabot.Plugin.Novelty.Quote
  ( quotePlugin
  )
where
...
import           Data.Fortune                   ( resolveFortuneFiles
                                                , randomFortune
                                                , FortuneType(All)
                                                )
...
import           Text.Regex.TDFA                ( captureGroups
                                                , newSyntax
                                                , caseSensitive
                                                , match
                                                , defaultExecOpt
...
quotePlugin = newModule
  ...
  , moduleCmds      =
    return
      ...
      , (command "forget") { help    = say "forget nick quote.  Delete a quote"
                           , process = runForget . strip isSpace
                           }
      , (command "ghc") { help    = say "ghc. Choice quotes from GHC."
                        , process = const (fortune ["ghc"])
                        }
      , (command "fortune") { help    = say "fortune. Provide a random fortune"
                            , process = const (fortune [])
                            }
      , (command "yow") { help    = say "yow. The zippy man."
                        , process = const (fortune ["zippy"])
                        }
      , (command "arr") { help    = say "arr. Talk to a pirate"
                        , process = const (fortune ["arr"])
                        }
      , (command "yarr") { help    = say "yarr. Talk to a scurvy pirate"
                         , process = const (fortune ["arr", "yarr"])
                         }
      , (command "keal") { help    = say "keal. Talk like Keal"
      ...
      , (command "brain") { help    = say "brain. Pinky and the Brain"
      ...
      , (command "protontorpedo") { help    = say "protontorpedo is silly"
                                  , process = const (fortune ["protontorpedo"])
                                  }
      , (command "nixon") { help    = say "Richard Nixon's finest."
                          , process = const (fortune ["nixon"])
                          }
      , (command "farber") { help    = say
...
runForget str
  ...
  | otherwise = do
    ss <- withMS $ \fm writer -> do
      ...
      writer fm'
      return ss
    say $ if P.pack q `elem` ss then "Done." else "No match."
 where
  (nm, rest) = break isSpace str
  q          = drop 1 rest
...
runQuote str = say =<< search (P.pack nm) (P.pack pat) =<< readMS
 where
  (nm, p) = break isSpace str
  pat     = drop 1 p
...
search key pat db
 ...
 where
  ...
  match' p ss = do
    re <- makeRegexOptsM
      defaultCompOpt { caseSensitive = False, newSyntax = True }

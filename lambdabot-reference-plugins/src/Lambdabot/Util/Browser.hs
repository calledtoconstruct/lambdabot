...
import Network.HTTP (
  HandleStream,
  HeaderName (HdrContentType),
  Response (rspBody, rspCode, rspHeaders),
  getRequest,
  lookupHeader,
 )
import Network.URI (escapeURIString)
import Text.HTML.TagSoup (canonicalizeTags, innerText, parseTags)
...
rawPageTitle url = checkHTTPS $ do
 ...
 where
  ...
  checkHTTPS act
    | "https:" `isPrefixOf` map toLower url = return Nothing
    | otherwise = act
...
extractTitle = content . tags . decodeString
 where

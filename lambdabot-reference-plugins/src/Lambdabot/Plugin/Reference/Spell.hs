...
import Lambdabot.Plugin (
  ...
  LB,
  Module (contextual, moduleCmds, moduleDefState),
  ModuleT,
...
import Lambdabot.Util (io, listToStr, showClean)
...
import Text.Regex.TDFA (MatchResult (mrAfter), (=~~))
...
spellPlugin =
  newModule
    ...
    , contextual = \txt -> do
        ...
        when alive $ io (spellingNazi binary txt) >>= mapM_ say
    }
...
spellingNazi binary lin = fmap (take 1 . concat) (mapM correct (words lin))
 where
  correct word = do
    ...
    return $ ["Did you mean " ++ listToStr "or" var ++ "?" | not (null var || any (equating' (map toLower) word) var)]

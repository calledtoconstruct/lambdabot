...
import Lambdabot.Plugin (Command (help, process), Module (moduleCmds, moduleSerialize), command, newModule, readMS, readOnly, say)
import Lambdabot.Util (randomFailureMsg)
    ...
    { moduleCmds =
        return
          [ (command "src")
              { help = say helpStr
              , process = \key ->
                  readMS >>= \env -> case fetch (P.pack key) env of
...
fetch x m = M.lookup x m `mplus` M.lookup (P.concat [P.singleton '(', x, P.singleton ')']) m

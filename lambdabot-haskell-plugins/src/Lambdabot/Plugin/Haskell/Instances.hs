...
instanceP cls = string "instance " *> (try constrained <|> unconstrained) *> skipMany space *> anyChar `manyTill` end
 where
  constrained = noneOf "=" `manyTill` string ("=> " ++ cls)
...
parseInstance cls = fmap (strip isSpace) . eitherToMaybe . parse (instanceP cls) "GHCi output"
...
getInstances s cls
  ...
  | not classFound = ["Couldn't find class `" ++ cls ++ "'. Try ?instances-importing"]
  | otherwise = sort $ mapMaybe doParse (tail splut)
 where
  classFound = s =~ ("class.*" ++ cls ++ ".*where")
  splut = splitOn "instance" s -- splut being the past participle
  -- of 'to split', obviously. :)
  notOperator = all (\c -> isAlpha c || isSpace c || (c `elem` "()"))
  unbracket str
    | head str == '(' && last str == ')'
        && notElem ',' str
        && notOperator str
...
fetchInstances' cls mdls = do
  load <- findL_hs
  let s = unlines $ map unwords [[":l", load], ":m" : "+" : mdls, [":i", cls]]

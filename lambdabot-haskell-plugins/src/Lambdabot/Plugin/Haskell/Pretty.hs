...
import Lambdabot.Plugin (Cmd, Command (help, process), LB, Module (moduleCmds), ModuleT, command, newModule, say)
...
prettyCmd rest =
  let code = dropWhile (`elem` " \t>") rest
      ...
      result = case (parseModule (modPrefix1 ++ code ++ "\n"), parseModule (modPrefix2 ++ code ++ "\n")) of
        ...
        (ParseFailed locat msg, _) ->
          let (SrcLoc _ _ col) = locat
           in [show msg ++ " at column " ++ show (col - prefLen1)]
...
doPretty (Hs.Module _ _ _ decls) =
  let defaultLen = 4
   ...
   in -- TODO: prefixing with hashes is done, because i didn't find a way
      --   to disable the indentation filter of lambdabot only for this module...
      map (" " ++) . lines . concat . intersperse "\n" . map prettyDecl $ decls

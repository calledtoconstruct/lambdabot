...
freeTheorem name t =
  runIdentity $ do
    ...
    return (evalState (insertRn name name >> rename th') initRnSt)
 where
  v0 = EVar name
...
initRnSt =
 ...
 where
  suggestionsVal =
    map (: []) "xyzuvabcstdeilmnorw"
      ++ ['x' : show i | i <- [1 ..]]
  suggestionsList =
    map (: "s") "xyzuvabcstdeilmnorw"
      ++ ["xs" ++ show i | i <- [1 ..]]
  suggestionsFun =
    map (: []) "fghkpq"
      ++ ['f' : show i | i <- [1 ..]]
...
lookupRn old = do
  m <- gets gamma
  return $ fromMaybe old (M.lookup old m)
  ...
  do
    ...
    let thf =
          ThEqual
            ( EApp
                ( foldl
                    (\e ((f, _), _) -> EApp e (EVar f))
                    (EBuiltin $ BMapTuple len)
                    fts
                )
                e1
            )
    ...
    return
      ( foldr
          (\((f, t), e1) e2 -> ThForall f t (ThImplies e1 e2))
          thf
          fts
      )
  ...
  do
    ...
    return
      ( ThForall
          ...
          (TyArr t1 t2)
          ( ThImplies
              (ThForall x t1 (ThForall y t2 (ThImplies p1 p2)))
              p3
          )
      )
...
freeTheorem' env e1 e2 t' = error "Sorry, this type is too difficult for me."

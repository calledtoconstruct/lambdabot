...
import Lambdabot.Plugin.Haskell.Free.Expr (
  Builtin (BId, BMap),
  Expr (EApp, EBuiltin, EVar, EVarOp),
  ExprCtx (ECAppR, ECDot),
  Fixity (FR),
  Var,
  applySimplifierExpr,
  exprSubst,
...
import Lambdabot.Plugin.Haskell.Free.Type (Type)
...
peepholeSimplifyTheorem' (ThForall v t p) =
  if varInTheorem v p then ThForall v t p else p
    ...
    case (traverseRight ECDot e1, traverseRight ECDot e2) of
      ((ctx1, EVar v1), (ctx2, EVar v2))
        | v1 == v2 && v1 `elem` map fst vts
            && not (varInCtx v1 ctx1)
            && not (varInCtx v2 ctx2) ->
          tryCurrying''
            vts
            ( ThEqual
                (untraverse ctx1)

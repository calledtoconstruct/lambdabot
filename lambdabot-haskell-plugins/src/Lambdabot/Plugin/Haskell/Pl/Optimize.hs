...
module Lambdabot.Plugin.Haskell.Pl.Optimize (optimize) where

import Lambdabot.Plugin.Haskell.Pl.Common (Expr (..), MonadPlus (..), fromJust, isJust, (&&&))
import Lambdabot.Plugin.Haskell.Pl.PrettyPrinter ()
import Lambdabot.Plugin.Haskell.Pl.Rules (RewriteRule (..), fire, rules)
...
-}
sizeExpr' :: Expr -> Size
sizeExpr' e = fromIntegral (length $ show e) + adjust e
 where
...
optimize e = result
 where
  result :: [Expr]
  result =
    map (snd . fromJust) . takeWhile isJust
      . iterate (>>= simpleStep)
      $ Just (sizeExpr' e, e)
  ...
  simpleStep t = do
    let chn = let ?first = True in step (snd t)
        ...
        new =
          filter (\(x, _) -> x < fst t) . map (sizeExpr' &&& id) $
            snd t : chn ++ chnn
    listToMaybe new
...
rewDeep rule e =
  rew rule e `mplus` case e of
    ...
    App e1 e2 ->
      ((`App` e2) `map` rewDeep rule e1)
        `mplus` ((e1 `App`) `map` rewDeep rule e2)

...
import Control.Monad.State ( MonadState(put, state, get), State, gets, evalState )
...
unLet (Let ds e) = unLet $
  Lambda (tupleP $ declName `map` dsYes) (Let dsNo e) `App`
    (fix' `App` Lambda (tupleP $ declName `map` dsYes) (tuple  $ declExpr `map` dsYes))
    where
...
alphaRename e = alpha e `evalState` M.empty where
  ...
  alpha (Var f v)     = do gets $ Var f . fromMaybe v . M.lookup v;

...
module Lambdabot.Plugin.Haskell.Pl.Transform (transform) where
...
import Data.Graph (flattenSCC, flattenSCCs, stronglyConnComp)
...
unLet (Let ds e) =
  unLet $
    Lambda (tupleP $ declName `map` dsYes) (Let dsNo e)
      `App` (fix' `App` Lambda (tupleP $ declName `map` dsYes) (tuple $ declExpr `map` dsYes))
 where
...
alphaRename e = alpha e `evalState` (M.empty, 0)
 where
  alpha :: Expr -> State Env Expr
  alpha (Var f v) = do (fm, _) <- get; return $ Var f $ fromMaybe v (M.lookup v fm)
  ...
  inEnv f = state $ \s -> (evalState f s, s)
...
transform' (Lambda (PTuple p1 p2) e) =
  transform' $
    Lambda (PVar "z") $
      Lambda p1 (Lambda p2 e) `App` f `App` s
 where
...
transform' (Lambda (PVar v) e) = transform' $ getRidOfV e
 where
  getRidOfV (Var f v')
    | v == v' = id'
    | otherwise = const' `App` Var f v'
  getRidOfV l@(Lambda pat _) = assert (not $ v `occursP` pat) $ getRidOfV $ transform' l

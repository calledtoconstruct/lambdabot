...
module Lambdabot.Plugin.Haskell.Pl.Test where
...
import Lambdabot.Plugin.Haskell.Pl.Parser
...
arbVar =
  oneof
    [ (Var Pref . return) `fmap` choose ('a', 'z')
    , (Var Inf . return) `fmap` elements (opchars \\ "=")
    ]

arbPat :: Gen Pattern
arbPat = sized $ \size ->
  let spat = resize (size `div` 5) arbPat
   in frequency $
        zip
          [1, size, size]
...
subExpr (App e1 e2) =
  [e1, e2]
    ++ App e1 `map` subExpr e2
    ++ (`App` e2) `map` subExpr e1
subExpr Let{} = bt

  ...
  showsPrec _ (List es)
    ...
    | otherwise =
      ('[' :)
        ...
        . (']' :)
   where
    fromSVar (SVar str) = Just str
    fromSVar _ = Nothing
  showsPrec _ (Enum fr tn to) =
    ('[' :) . shows fr
...
instance Show Pattern where
  ...
  showsPrec _ (PTuple p1 p2) =
    showParen True $
      shows p1 . (", " ++) . shows p2
  showsPrec p (PCons p1 p2) =
    showParen (p > 5) $

...
data RewriteRule
  = -- | A 'Rewrite' rule, rewrite the first to the second
    --   'Rewrite's can contain 'Hole's
    RR Rewrite Rewrite
  | -- | Haskell function as a rule, applied to subexpressions
    CRR (Expr -> Maybe Expr)
  | -- | Like Up, but applied to subexpressions
    Down RewriteRule RewriteRule
  | -- | Apply the first rule, then try the second rule on the first result
    --   if it fails, returns the result of the first rule
    Up RewriteRule RewriteRule
  | -- | Use all rules
    Or [RewriteRule]
  | -- | Try the first rule, if it fails use the second rule
    OrElse RewriteRule RewriteRule
  | -- | Apply the first rule, apply the second rule to the result
    Then RewriteRule RewriteRule
  | -- | Optionally apply the rewrite rule, Opt x == Or [identity,x]
    Opt RewriteRule
  | -- | Apply the second rule only if the first rule has some results
    If RewriteRule RewriteRule
  | -- | Apply the rule only in the first pass
    Hard RewriteRule
...
rrList r1 r2 = zipWith RR (getVariants r1') (getVariants r2')
 where
  r1' = getRewrite r1
  r2' = getRewrite r2

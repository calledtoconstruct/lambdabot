...
simplifies =
  Or
    ...
    , -- fst (x,y) --> x
      rr
        (\x y -> fstE `a` (commaE `a` x `a` y))
        const
    , -- snd (x,y) --> y
      rr
        (\x y -> sndE `a` (commaE `a` x `a` y))
        (\_ y -> y)
    , -- head (x:xs) --> x
      rr
        (\x xs -> headE `a` (consE `a` x `a` xs))
        const
    , -- tail (x:xs) --> xs
      rr
        (\x xs -> tailE `a` (consE `a` x `a` xs))
        (\_ xs -> xs)
    , -- uncurry f (x,y) --> f x y
      rr1
        (\f x y -> uncurryE `a` f `a` (commaE `a` x `a` y))
        (\f x y -> f `a` x `a` y)
    , -- uncurry (,) --> id
      rr
        (uncurryE `a` commaE)
    ...
    , -- uncurry f . s (,) g --> s f g
      rr1
        (\f g -> (uncurryE `a` f) `c` (sE `a` commaE `a` g))
        (\f g -> sE `a` f `a` g)
    , -- curry fst --> const
    ...
    , -- s f g x --> f x (g x)
      rr0
        (\f g x -> sE `a` f `a` g `a` x)
        (\f g x -> f `a` x `a` (g `a` x))
    , -- flip f x y --> f y x
      rr0
        (\f x y -> flipE `a` f `a` x `a` y)
        (\f x y -> f `a` y `a` x)
    , -- flip (=<<) --> (>>=)
      rr0
        (flipE `a` extE)
    ...
    , -- fmap f . fmap g -> fmap (f . g)
      rr0
        (\f g -> fmapE `a` f `c` fmapE `a` g)
        (\f g -> fmapE `a` (f `c` g))
    , -- map f . map g -> map (f . g)
      rr0
        (\f g -> mapE `a` f `c` mapE `a` g)
        (\f g -> mapE `a` (f `c` g))
    ]
...
rules =
  Or
    ...
    , -- (=<<) f (return x) -> f x
      rr
        (\f x -> extE `a` f `a` (returnE `a` x))
        a
    , -- (=<<) ((=<<) f . g) --> (=<<) f . (=<<) g
      rr
        (\f g -> extE `a` ((extE `a` f) `c` g))
    ...
    , -- flip (.) f . flip id --> flip f
      rr
        (\f -> (flipE `a` compE `a` f) `c` (flipE `a` idE))
        (\f -> flipE `a` f)
    , -- flip (.) f . flip flip --> flip (flip . f)
      rr
        (\f -> (flipE `a` compE `a` f) `c` (flipE `a` flipE))
        (\f -> flipE `a` (flipE `c` f))
    , -- flip (flip (flip . f) g) --> flip (flip . flip f) g
      rr1
        (\f g -> flipE `a` (flipE `a` (flipE `c` f) `a` g))
    ...
    , -- s const x y --> y
      rr
        (\x y -> sE `a` constE `a` x `a` y)
        (\_ y -> y)
    , -- s (const . f) g --> f
      rr1
        (\f g -> sE `a` (constE `c` f) `a` g)
        const
    , -- s (const f) --> (.) f
      rr
        (\f -> sE `a` (constE `a` f))
    ...
    , -- (`ap` f) . const . h --> (. f) . h
      rr
        (\f g h -> (flipE `a` sE `a` f) `c` (flipE `a` compE `a` g) `c` constE `c` h)
        (\f _ h -> (flipE `a` compE `a` f) `c` h)
    , -- s (f . fst) snd --> uncurry f
      rr
        (\f -> sE `a` (f `c` fstE) `a` sndE)
        (\f -> uncurryE `a` f)
    , -- fst (join (,) x) --> x
      rr
        (\x -> fstE `a` (joinE `a` commaE `a` x))
        id
    , -- snd (join (,) x) --> x
      rr
        (\x -> sndE `a` (joinE `a` commaE `a` x))
        id
    , -- The next two are `simplifies', strictly speaking, but invoked rarely.
      ...
      -- curry (uncurry f) --> f
      rr
        (\f -> curryE `a` (uncurryE `a` f))
        id
    , -- uncurry (curry f) --> f
      rr
        (\f -> uncurryE `a` (curryE `a` f))
        id
    , -- (const id . f) --> const id
      rr
        (\f -> (constE `a` idE) `c` f)
    ...
    , -- (. f) . const --> const
      rr
        (\f -> (flipE `a` compE `a` f) `c` constE)
        (const constE)
    , -- (. f) . const . g --> const . g
      rr
        (\f g -> (flipE `a` compE `a` f) `c` constE `c` g)
        (\_ g -> constE `c` g)
    , -- fix f --> f (fix x)
      Hard $
        rr0
    ...
    , -- not (x == y) -> x /= y
      rr2
        (\x y -> notE `a` (equalsE `a` x `a` y))
        (\x y -> nequalsE `a` x `a` y)
    , -- not (x /= y) -> x == y
      rr2
        (\x y -> notE `a` (nequalsE `a` x `a` y))
        (\x y -> equalsE `a` x `a` y)
    , If (Or [rr plusE plusE, rr minusE minusE, rr multE multE]) $
        down $
          Or
            [ -- 0 + x --> x
              rr
            ...
            , -- x - y + y --> x
              rr
                (\y x -> plusE `a` (minusE `a` x `a` y) `a` y)
                (\_ x -> x)
            , -- x + y - y --> x
              rr
                (\y x -> minusE `a` (plusE `a` x `a` y) `a` y)
                (\_ x -> x)
            , -- x + (y - z) --> x + y - z
              rr
                (\x y z -> plusE `a` x `a` (minusE `a` y `a` z))
                (\x y z -> minusE `a` (plusE `a` x `a` y) `a` z)
            , -- x - (y + z) --> x - y - z
              rr
                (\x y z -> minusE `a` x `a` (plusE `a` y `a` z))
                (\x y z -> minusE `a` (minusE `a` x `a` y) `a` z)
            , -- x - (y - z) --> x + y - z
              rr
                (\x y z -> minusE `a` x `a` (minusE `a` y `a` z))
                (\x y z -> minusE `a` (plusE `a` x `a` y) `a` z)
            ]
    , Hard onceRewrites
    , -- join (fmap f x) --> f =<< x
      rr
        (\f x -> joinE `a` (fmapE `a` f `a` x))
        (\f x -> extE `a` f `a` x)
    , -- (=<<) id --> join
      rr (extE `a` idE) joinE
    , -- join --> (=<<) id
    ...
    , -- join (return x) --> x
      rr
        (\x -> joinE `a` (returnE `a` x))
        id
    , -- (return . f) =<< m --> fmap f m
      rr
        (\f m -> extE `a` (returnE `c` f) `a` m)
        (\f m -> fmapIE `a` f `a` m)
    , -- (x >>=) . (return .) . f  --> flip (fmap . f) x
      rr
        (\f x -> bindE `a` x `c` (compE `a` returnE) `c` f)
        (\f x -> flipE `a` (fmapIE `c` f) `a` x)
    , -- (>>=) (return f) --> flip id f
      rr
        (\f -> bindE `a` (returnE `a` f))
        (\f -> flipE `a` idE `a` f)
    ...
    , -- liftM2 f (return x) --> fmap (f x)
      rr
        (\f x -> liftM2E `a` f `a` (returnE `a` x))
        (\f x -> fmapIE `a` (f `a` x))
    , -- f `fmap` return x --> return (f x)
      rr
        (\f x -> fmapE `a` f `a` (returnE `a` x))
        (\f x -> returnE `a` (f `a` x))
    , -- (=<<) . flip (fmap . f) --> flip liftM2 f
      Hard $
        rr
          (\f -> extE `c` flipE `a` (fmapE `c` f))
    ...
    , -- map f (zip xs ys) --> zipWith (curry f) xs ys
      Hard $
        rr
          (\f xs ys -> mapE `a` f `a` (zipE `a` xs `a` ys))
    ...
    , -- (x >>=) . flip (fmap . f) -> liftM2 f x
      rr
        (\f x -> bindE `a` x `c` flipE `a` (fmapE `c` f))
        (\f x -> liftM2E `a` f `a` x)
    , -- (f =<< m) x --> f (m x) x
      rr0
        (\f m x -> extE `a` f `a` m `a` x)
        (\f m x -> f `a` (m `a` x) `a` x)
    , -- (fmap f g x) --> f (g x)
      rr0
        (\f g x -> fmapE `a` f `a` g `a` x)
        (\f g x -> f `a` (g `a` x))
    , -- return x y --> y
      rr
        (\y x -> returnE `a` x `a` y)
    ...
    , -- liftM2 f g h x --> g x `h` h x
      rr0
        (\f g h x -> liftM2E `a` f `a` g `a` h `a` x)
    ...
    , -- experimental support for Control.Arrow stuff
      ...
      -- uncurry ((. g) . (,) . f) --> f *** g
      rr
        (\f g -> uncurryE `a` ((flipE `a` compE `a` g) `c` commaE `c` f))
        (\f g -> crossE `a` f `a` g)
    , -- uncurry ((,) . f) --> first f
      rr
        (\f -> uncurryE `a` (commaE `c` f))
        (\f -> firstE `a` f)
    , -- uncurry ((. g) . (,)) --> second g
      rr
        (\g -> uncurryE `a` ((flipE `a` compE `a` g) `c` commaE))
        (\g -> secondE `a` g)
    , -- I think we need all three of them:
      -- uncurry (const f) --> f . snd
      rr
        (\f -> uncurryE `a` (constE `a` f))
        (`c` sndE)
    , -- uncurry const --> fst
      rr
        (uncurryE `a` constE)
    ...
    , -- list destructors
      Hard $
        If (Or [rr consE consE, rr nilE nilE]) $
          Or
            [ down $
                Or
                  [ -- length [] --> 0
              ...
              down $
                Or
                  [ -- map f (x:xs) --> f x: map f xs
                    rr
                      (\f x xs -> mapE `a` f `a` (consE `a` x `a` xs))
                      (\f x xs -> consE `a` (f `a` x) `a` (mapE `a` f `a` xs))
                  , -- fmap f (x:xs) --> f x: Fmap f xs
                    rr
                      (\f x xs -> fmapE `a` f `a` (consE `a` x `a` xs))
              ...
              down $
                Or
                  [ -- foldr f z (x:xs) --> f x (foldr f z xs)
                    rr
                      (\f x xs z -> (foldrE `a` f `a` z) `a` (consE `a` x `a` xs))
                      (\f x xs z -> (f `a` x) `a` (foldrE `a` f `a` z `a` xs))
                  , -- foldr f z [] --> z
                    rr
                      (\f z -> foldrE `a` f `a` z `a` nilE)
                      (\_ z -> z)
                  ]
            , -- foldl elimination
              down $
                Opt (CRR $ assocL ["."])
                  `Then` Or
                    ...
                    , -- foldl1 f (x:xs) --> foldl f x xs
                      rr
                        (\f x xs -> foldl1E `a` f `a` (consE `a` x `a` xs))
                        (\f x xs -> foldlE `a` f `a` x `a` xs)
                    , -- foldl f z (x:xs) --> foldl f (f z x) xs
                      rr
                        (\f z x xs -> (foldlE `a` f `a` z) `a` (consE `a` x `a` xs))
                        (\f z x xs -> foldlE `a` f `a` (f `a` z `a` x) `a` xs)
                    , -- foldl f z [] --> z
                      rr
                        (\f z -> foldlE `a` f `a` z `a` nilE)
                        (\_ z -> z)
                    , -- special rule:
                      -- foldl f z [x] --> f z x
                      rr
                        (\f z x -> foldlE `a` f `a` z `a` (returnE `a` x))
                        (\f z x -> f `a` z `a` x)
                    , rr
                        (\f z x -> foldlE `a` f `a` z `a` (consE `a` x `a` nilE))
                        (\f z x -> f `a` z `a` x)
                    ]
                  `OrElse` (
                             -- (:) x --> (++) [x]
    ...
    , Up (CRR (commutative commutativeOps)) $
        down $
          Or
            [ CRR $ assocL assocLOps

...
getExpr (TLD True (Define foo e)) = (Let [Define foo e] (Var Pref foo), TLD False . Define foo)
...
sizeExpr (Let ds e) = 1 + sum (map sizeDecl ds) + sizeExpr e
 where

...
import Lambdabot.Config.Haskell (hoogleBinary)
  ...
  newModule
    ...
    , moduleCmds =
        return
          ...
          , (command "hoogle+") -- TODO: what does this really do?  give it a proper help msg
              { help = say "hoogle <expr>. Haskell API Search for either names, or types."
              , process = \_ -> do
...
hoogle binary s = do
 ...
 where
  result [] [] = ["A Hoogle error occurred."]
  result [] ys = [ys]
  result xs _ =

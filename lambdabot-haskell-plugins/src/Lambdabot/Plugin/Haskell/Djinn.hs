...
djinnPlugin =
  newModule
    ...
    , moduleCmds =
        return
          [ (command "djinn")
              { help = mapM_ say ["djinn <type>.", "Generates Haskell code from a type.", "https://github.com/augustss/djinn"]
...
djinnNamesCmd = do
  (prelude, st) <- lift getSavedEnv
  let names = unwords (concatMap extractNames $ prelude ++ st)
  say names
 where
  extractNames = filter (isUpper . head) . unfoldr (\x -> case x of _ : _ -> listToMaybe (lex x); _ -> Nothing)
...
getDjinnEnv (prel, env') = do
  ...
  return $ fmap readEnv env
 where
  readEnv o =
    let new = filter (`notElem` prel) . nub . lines $ o
     in (prel, new)
...
getDjinnVersion = do
 ...
 where
  ...
  extractVersion str = fromMaybe "Unknown" (str =~~ "version [0-9]+(-[0-9]+)*")
...
clean_ s
  | Just mr <- s =~~ prompt = mrBefore mr ++ mrAfter mr
  | otherwise = s
 where
  prompt = "(Djinn> *)+"

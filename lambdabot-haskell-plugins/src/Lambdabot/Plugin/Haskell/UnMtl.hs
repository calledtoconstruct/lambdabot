...
import Control.Monad (MonadPlus (mplus), ap, liftM, liftM2)
...
mkPfun n cont = PMonad n (Just msg) (Just fun)
 where
  fun p = cont p (TyApp n (pResult p))
...
($$) f x =
  PMonad
    ...
    , pError =
        pError f
          `mplus` if isFunction (pResult f) -- ignore errors in x, the type constructor f might have a higher kind and ignore x
            then Nothing
            else Just $ "`" ++ prettyPrintInLine (pResult f) ++ "' is not a type function."
    , pFun = Nothing
    }
 where
...
tuple :: [PType] -> PType
tuple = fmap (TyTuple Boxed . concatMap unpack) . sequence
 where
...
mtlParser input = do
 ...
 where
  ...
  liftE (ParseFailed _src str) = error str

...
type TimeMap =
  M.Map
    Nick -- the person who's time we requested
...
doLocalTime :: (MonadLBState m, LBState m ~ M.Map Nick [Nick]) => String -> Cmd m ()
doLocalTime [] = do
  n <- getSender
  doLocalTime (nName n)
doLocalTime rawWho = do
  whoAsked <- getTarget
  whoToPing <- readNick $ takeWhile (/= ' ') rawWho
  me <- getLambdabotName
  if whoToPing /= me
    then do
...
doReply :: (MonadLBState m, LBState m ~ M.Map Nick [Nick]) => String -> Cmd m ()
doReply text = do
  let (whoGotPinged', time') = break (== ':') text
      time = drop 1 time'
  whoGotPinged <- readNick whoGotPinged'

  targets <- withMS $ \st set -> case M.lookup whoGotPinged st of
    Nothing -> return []
    Just xs -> do
      set (M.insert whoGotPinged [] st) -- clear the callback state
      return xs
  whoGotPinged'' <- showNick whoGotPinged
  let txt = "Local time for " ++ whoGotPinged'' ++ " is " ++ time
  lb $ forM_ targets $ flip ircPrivmsg' txt

...
main = do
  ...
  let ts = parseTags s
  let das =
        [ (map toDot mod, intercalate "-" pkg)
        | TagOpen
            "a"
            [ ( "href"
                , splitOn "/" ->
                    [init2 . splitOn "-" -> pkg, stripSuffix ".html" -> Just mod]
                )
              ] <-
            ts
        , any isUpper mod
  ...
  print $ text header $+$ ppPkgs (M.toList uniq) $+$ ppAssocs uniq das
...
ppAssocs u das =
  text "docAssocs :: M.Map P.ByteString (P.ByteString, P.ByteString)"
    $$ hang
      (text "docAssocs = {-# SCC \"Dummy.DocAssocs\" #-} M.fromList")
      2
      (brackets $ vcat $ punctuate comma $ map (ppAssoc u) das)

ppAssoc u (a, b) =
  let Just i = M.lookup b u
   in parens $
        text "P.pack" <+> doubleQuotes (text (map toLower a))
          <+> comma
          <+> parens
            (text "P.pack" <+> doubleQuotes (text a) <> comma <+> v i)

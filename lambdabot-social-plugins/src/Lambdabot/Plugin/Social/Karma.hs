...
import Lambdabot.Compat.FreenodeNick (freenodeNickMapSerial)
    ...
    , -- nick++($| )
      contextual = \text -> withMsg $ \_ -> do
        ...
        let ws = words text
            ...
            incs = match "++"
            match m =
              mapM readNick . filter okay . map (reverse . drop 2)
                . filter (isPrefixOf m)
                . map reverse
                $ ws
...
tellKarma sender nick = do
  ...
  say $
    concat
      [ if E.mononickToPolynick sender == nick then "You have" else nickStr ++ " has"
      , " a karma of "
      , show karma
      ]

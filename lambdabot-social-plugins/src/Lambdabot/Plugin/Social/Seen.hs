...
import Lambdabot.Plugin.Social.Seen.StopWatch (StopWatch (Stopped), startWatch, zeroWatch)
import Lambdabot.Plugin.Social.Seen.UserStatus (Channel, UserStatus (..), updateJ, updateNP)
import Lambdabot.Util (io, listToStr)
    ...
    , moduleExit =
        do
          ...
          unless (null chans) $ do
            ...
            modifyMS $ second (botPart clockTime (map packNick chans))
...
doUsers rest = withMsg $ \msg -> do
  ...
  let n = fromMaybe 1 (M.lookup who m)
...
replyImHere :: Nick -> M.Map P.ByteString UserStatus -> (Channel -> String) -> ([String], Bool)
...
replyCurrent :: String -> Nick -> M.Map a UserStatus -> (a -> String) -> ([String], Bool)
...
replySpecific :: (G.Message p, Monad m) => p -> String -> Nick -> M.Map P.ByteString UserStatus -> (Channel -> String) -> ClockTime -> (m String, Bool)
replySpecific msg nick' pnick seenFM upAndShow now =
 ...
 where
  nickPresent maybeClockTime cs =
    ...
    , case maybeClockTime of
        ...
        Just (clockTime, missed) ->
         ...
         where
          lastSpoke = clockDifference clockTime
    ]
  ...
  clockDifference past
    ...
    | otherwise = diff ++ " ago"
   where
    diff = timeDiffPretty . diffClockTimes now $ past
